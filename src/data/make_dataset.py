...
def encode_outcome(df: pd.DataFrame) -> pd.DataFrame:
    """Encode concurrent undernutrition categories from WHO Z-score thresholds.

    Class codes: N (normal), U (underweight), S (stunting), W (wasting),
    US (underweight+stunting), UW (underweight+wasting), USW (all three).
    """
    ...
    # SW (stunting & wasting) is omitted because it is absent in the manuscript (and data).
